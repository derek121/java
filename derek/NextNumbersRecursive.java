...
public class NextNumbersRecursive {
	...
	private List<Integer> find2(final LazyGenerator generator, final int howMany,
		...
		/*
		 * To make this non-tail recursive and see that it can cause a stack overflow, we can 
		 * comment the above return, and instead calculate the return the same way, do a dummy 
		 * operation that the compiler won't remove, then do the return. Calling find with a 
		 * start value of 1234567890, for example, will overflow (as will smaller values).
		 * 
		List<Integer> ret = find2(generator.tail(), howMany, accumulator);
